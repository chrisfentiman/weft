...
mod tests {
    ...
    fn make_provider_registry(provider: impl Provider + 'static) -> Arc<ProviderRegistry> {
        let mut providers = HashMap::new();
        providers.insert(
            "test-model".to_string(),
            Arc::new(provider) as Arc<dyn weft_llm::Provider>,
        );
    ...
    fn make_weft_service(provider: impl Provider + 'static) -> Arc<WeftService> {
        let config = test_config();

        let provider_registry = make_provider_registry(provider);
        ...
        let services = Arc::new(Services {
            ...
            commands: Arc::new(MockCommandRegistry)
