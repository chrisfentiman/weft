...
//!
//! The EventLog is the durable event store. Events are appended, never modified.
//! Implementations live in separate crates:
//! - `weft_eventlog_memory::InMemoryEventLog`: for tests and local dev.
//! - `weft_eventlog_postgres::PostgresEventLog`: for production.
...
///
/// This trait lives in `weft_reactor_trait`. Implementations live in separate crates:
