...
impl Reactor {
    ...
    pub async fn execute(
        ...
        self.run_pre_loop(&mut lctx).await?;
        ...
        if let Some(early_result) = self
            ...
            .await?
        {
            // Dispatch loop returned early (cancellation). Return immediately.
            return Ok((early_result, event_tx));
        }

        // ── Phase 3: Post-loop activities ─────────────────────────────────────
        self.run_post_loop(&mut lctx).await?;

        // ── Finalization ──────────────────────────────────────────────────────
        let duration_ms = lctx.state.start_time.elapsed().as_millis() as u64;
        let generation_calls_used =
            lctx.state.budget.max_generation_calls - lctx.state.budget.remaining_generation_calls;
